...
public class TitleItemDecoration extends RecyclerView.ItemDecoration{
    ...
    private static int COLOR_TITLE_BG = Color.parseColor("#FFcccccc");
    private static int COLOR_TITLE_FONT = Color.parseColor("#FF000000");
    private static int mTitleFontSize;//title字体大小


    public TitleItemDecoration(Context context, List<CityBean.CtsBean> datas) {
        super();
        mDatas = datas;
        mPaint = new Paint();
        mBounds = new Rect();
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (position > -1) {
                if (position == 0) {//等于0肯定要有title的
                    ...
                    if (null != mDatas.get(position).getFirstLetter() && !mDatas.get(position).getFirstLetter().equals(mDatas.get(position - 1).getFirstLetter())) {
...
/*
        ...
        mPaint.getTextBounds(mDatas.get(position).getFirstLetter(), 0, mDatas.get(position).getFirstLetter().length(), mBounds);
        c.drawText(mDatas.get(position).getFirstLetter(), child.getPaddingLeft(), child.getTop() - params.topMargin - (mTitleHeight / 2 - mBounds.height() / 2), mPaint);
    }
    ...
    public void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.State state) {//最后调用 绘制在最上层
        ...
        String tag = mDatas.get(pos).getFirstLetter();
