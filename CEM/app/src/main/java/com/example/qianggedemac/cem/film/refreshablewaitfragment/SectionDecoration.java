...
public class SectionDecoration extends RecyclerView.ItemDecoration {
    ...
    private List<HeaderNameBean> dataLists;
    ...
    public SectionDecoration(List<HeaderNameBean> dataList, Context context, DecorationCallback decorationCallback) {
        Resources res = context.getResources();
        this.dataLists = dataList;
        this.mDecorationCallback = decorationCallback;
        //设置悬浮栏的画笔---paint
        paint = new Paint();
        paint.setColor(res.getColor(R.color.backgroundOne));

        //设置悬浮栏中文本的画笔
        textPaint = new TextPaint();
        textPaint.setAntiAlias(true);
        textPaint.setTextSize(DensityUtil.dp2px(context, 10));
        textPaint.setColor(Color.DKGRAY);
        textPaint.setTextAlign(Paint.Align.LEFT);
        fontMetrics = new Paint.FontMetrics();
        //决定悬浮栏的高度等
        topGap = res.getDimensionPixelSize(R.dimen.twenty);
        //决定文本的显示位置等
        alignBottom = res.getDimensionPixelSize(R.dimen.marginEight);
    ...
    private boolean isFirstInGroup(int pos) {
        if (pos == 0) {
            ...
            String prevGroupId = mDecorationCallback.getGroupId(pos - 1);
            String groupId = mDecorationCallback.getGroupId(pos);
